...
include_once('../../model/Produccion/Mostrar_Movimiento_General_Destilados_Cremas_Licores.php');
...
$spreadsheet->getProperties()->setTitle("Movimientos Destilados Lote ".$lote)->setCreator("Casa Mezcal Huitzila")
->setCategory("Movimientos Destilados")->setCompany("Casa Mezcal Huitzila")->setLastModifiedBy("Casa Mezcal Huitzila");

//establecer la hoja en la que vamos a trabajar
$spreadsheet->setActiveSheetIndex(0)->setTitle("Movimientos Destilados");
    ...
    if ($resultado) {
        ...
        for ($i = 0; $i < count($resultado); $i++) {
            ...
            if ($fila['Fecha'] == $fecha) {
                ...
                $noLote = $fila["Lote"];
                $analisisFQ = $analisis;
                $categoria = $fila["NombreDestilado"];
...
header('Content-Disposition: attachment;filename="Movimientos Destilados Lote '.$lote. '.Xlsx"');
